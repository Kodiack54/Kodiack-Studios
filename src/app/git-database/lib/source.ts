...
export async function getGitSummary(filters?: SummaryFilters): Promise<GitSummaryResponse> {
  try {
    ...
    for (const [key, pair] of repoPairs) {
      ...
      repos.push({
        ...
        registry: regEntry ? {
          ...
          is_ai_team: regEntry.is_ai_team,
        } : undefined,
    ...
    filtered.sort((a, b) => {
      const order = { yellow: 0, orange: 1, red: 2, gray: 3, green: 4 };
    ...
    return {
      success: false,
      repos: [],
      counts: { total: 0, green: 0, yellow: 0, orange: 0, red: 0, gray: 0 },
      timestamp: new Date().toISOString(),
    };
  }
}
...
export async function getFamilySummary(): Promise<FamilySummaryResponse> {
  try {
    ...
    // Count by state
    const counts = { total: families.length, green: 0, yellow: 0, orange: 0, red: 0, gray: 0 };
